...
def test_full_pipeline():
    ...
    try:
        ...
        print(f"  OK Sensitivity computed for {len(sensitivities)} parameters")
    except Exception as e:
        print(f"  WARN Sensitivity estimation skipped: {e}")
        sensitivities = {}
    ...
    print(f"  OK Created {len(layer_configs)} layer configurations")
    ...
    try:
        ...
        print(f"  OK Surrogate initialized with {surrogate.get_buffer_size()} sample(s)")
        
        # Train surrogate
        surrogate.train(epochs=5, batch_size=4)
        print(f"  OK Surrogate trained")
        
    except Exception as e:
        print(f"  WARN Surrogate test skipped: {e}")
    ...
    if compressed_acc > 0:  # Model still produces output
        ...
        print("\nFAIL INTEGRATION TEST FAILED: Invalid accuracy")
        return 1
