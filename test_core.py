...
def test_quantization_uniform():
    ...
    print("  OK Uniform quantization works")
...
def test_quantization_logarithmic():
    ...
    assert q_tensor.shape == tensor.shape
    print("  OK Logarithmic quantization works")
...
def test_quantization_per_channel():
    ...
    assert q_tensor.shape == tensor.shape
    print("  OK Per-channel quantization works")
...
def test_quantization_learned():
    ...
    print("  OK Learned quantization works")
...
def test_pruning_magnitude():
    ...
    print(f"  OK Magnitude pruning works (NNZ: {original_nnz} -> {pruned_nnz})")
...
def test_pruning_gradient_based():
    ...
    print("  OK Gradient-based pruning works")
...
def test_fine_grained_pruning():
    ...
    assert pruned.shape == tensor.shape
    print("  OK Channel pruning works")
...
def test_kernel_config_compression():
    ...
    print(f"  OK Compression ratio: {ratio:.4f}")
...
def test_layer_config_aggregates():
    ...
    print(f"  OK Avg weight bits: {config.avg_weight_bits}")
    print(f"  OK Avg pruning ratio: {config.avg_pruning_ratio:.4f}")
...
def test_surrogate_basic():
    ...
    print("  OK Surrogate initialized successfully")
    ...
    print("  OK Added sample to surrogate")
...
def test_sensitivity_gradient():
    ...
    try:
        ...
        assert len(scores) > 0
        print(f"  OK Computed sensitivity for {len(scores)} parameters")
    except Exception as e:
        print(f"  WARN Sensitivity test skipped: {str(e)}")
...
def test_quantization_types():
    ...
    for qtype in ['INT', 'FLOAT']:
        ...
        assert q_tensor.shape == tensor.shape
        print(f"  OK {qtype} quantization works")
