...
class CNNCompressionTrainer(BaseCompressionTrainer):
    ...
    def run_hierarchical_compression(self) -> Dict[int, LayerConfig]:
        ...
        Main algorithm:
        1. Compute sensitivity analysis
        2. Initialize and pretrain surrogate
        3. For each cycle:
           - Create and train HLA (budget allocation)
           - For each block, create and train LLA ensemble
           - Extract configurations from best agent
           - Apply compression to model
           - Fine-tune compressed model
           - Update surrogate with results
           - Log metrics and visualization
        ...
        # Final visualization
        self.logger.log("\n" + "="*80)
        self.logger.log("Generating final visualizations...")
    ...
    def generate_all_visualizations(self) -> None:
        ...
        try:
            ...
            # 2. Accuracy vs size (Pareto front)
...
def create_cnn_trainer(model_name: str = "resnet18",
                       dataset: str = "cifar10",
                       num_classes: int = 10,
                       compression_goal: Optional[CompressionGoal] = None,
                       experiment_name: str = "cnn_compression",
                       device: str = "cuda") -> CNNCompressionTrainer:
    """
    Create a CNNCompressionTrainer with default or custom configuration.
    
    Args:
        model_name: Model name (timm supported)
    ...
    Returns:
        Initialized CNNCompressionTrainer
    """
    ...
    return CNNCompressionTrainer(config, logger)
