...
class ExperimentLogger:
    ...
    def log_config(self, config: Dict[str, Any]) -> None:
        """Log experiment configuration and persist to JSON."""
    ...
    def log_metric(self, key: str, value: Any, step: Optional[int] = None) -> None:
        """
        Log a metric (latest value stored directly, history in _history).
        """
        self.metrics[key] = value
        history = self.metrics.setdefault('_history', {})
    ...
    def log_cycle_result(self, cycle: int, data: Dict[str, Any]) -> None:
        ...
        for key, value in data.items():
            if isinstance(value, float):
                ...
                self.log(f"  {key}: {value}")
        self.metrics.setdefault('cycle_history', []).append({'cycle': cycle, **data})
    ...
    def log_surrogate_prediction(self,
                                 cycle: int,
                                 predicted: Optional[float] = None,
                                 actual: Optional[float] = None,
                                 predicted_accuracy: Optional[float] = None,
                                 actual_accuracy: Optional[float] = None) -> None:
        """Log surrogate predictions vs actual (supports legacy arg names)."""
        pred = predicted if predicted is not None else predicted_accuracy
        act = actual if actual is not None else actual_accuracy

        if pred is None or act is None:
            raise ValueError("predicted/actual values are required")

        error = abs(pred - act)
        self.log(f"Surrogate [Cycle {cycle}] Pred: {pred:.2f}%, Actual: {act:.2f}%, "
                 f"Error: {error:.2f}%")
        self.metrics.setdefault('surrogate_predictions', []).append({
            'cycle': cycle,
...
class DataManager:
    ...
    @staticmethod
    def get_cached_batches(dataloader, device: str, num_batches: int = 3):
        """
        Cache a few batches on the target device for fast repeated evaluation.
        ...
        Returns:
            (inputs, targets) concatenated tensors
        """
        import torch
        ...
        if not inputs_list:
            raise ValueError("No batches available to cache")
