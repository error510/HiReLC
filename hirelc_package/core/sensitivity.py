...
class SNIPSensitivityEstimator(BaseSensitivityEstimator):
    """
    SNIP-based sensitivity estimation using |w * grad|.

    This is a single-shot method that measures parameter saliency based
    on the gradient of the loss with respect to weights.
    """

    def estimate_importance(self, num_samples: int = 1) -> Dict[int, float]:
        """Estimate importance via SNIP saliency."""
        self.model.eval()
        saliency = {}
        criterion = nn.CrossEntropyLoss()
        ...
        for batch_idx, (inputs, targets) in enumerate(tqdm(self.dataloader,
            ...
            for name, param in self.model.named_parameters():
                if param.grad is None:
                    continue
                saliency[name] = saliency.get(name, 0.0) + (param.grad.data * param.data).abs().sum().item()
...
# Registry helpers for extensibility
SENSITIVITY_REGISTRY: Dict[str, Type[BaseSensitivityEstimator]] = {}


def register_sensitivity_estimator(name: str, estimator_cls: Type[BaseSensitivityEstimator]) -> None:
    """Register a custom sensitivity estimator by name."""
    key = name.strip().lower()
    if not key:
        raise ValueError("Estimator name must be non-empty")
    if not issubclass(estimator_cls, BaseSensitivityEstimator):
        raise TypeError("Estimator must inherit from BaseSensitivityEstimator")
    SENSITIVITY_REGISTRY[key] = estimator_cls


def get_sensitivity_estimator(name: Optional[str]) -> Type[BaseSensitivityEstimator]:
    """Lookup an estimator class by name with safe fallback."""
    key = (name or 'fisher').strip().lower()
    if key in SENSITIVITY_REGISTRY:
        return SENSITIVITY_REGISTRY[key]
    return SENSITIVITY_REGISTRY.get('gradient', GradientNormSensitivityEstimator)


def build_sensitivity_estimator(model: nn.Module, dataloader: DataLoader,
                                device: str = 'cuda', method: Optional[str] = None,
                                num_blocks: Optional[int] = None,
                                methods: Optional[List[str]] = None) -> BaseSensitivityEstimator:
    """
    Build a sensitivity estimator instance using registry lookup.

    For composite/ensemble methods, uses CompositeSensitivityEstimator with
    an optional list of method names.
    """
    key = (method or 'fisher').strip().lower()
    if key in ('composite', 'ensemble'):
        return CompositeSensitivityEstimator(
            model, dataloader, device=device, num_blocks=num_blocks, methods=methods
        )

    estimator_cls = get_sensitivity_estimator(key)
    if estimator_cls is FisherSensitivityEstimator:
        return estimator_cls(model, dataloader, device=device, num_blocks=num_blocks)
    return estimator_cls(model, dataloader, device=device)


# Register built-in estimators
register_sensitivity_estimator('fisher', FisherSensitivityEstimator)
register_sensitivity_estimator('hessian', HessianSensitivityEstimator)
register_sensitivity_estimator('gradient', GradientNormSensitivityEstimator)
register_sensitivity_estimator('snip', SNIPSensitivityEstimator)
register_sensitivity_estimator('composite', CompositeSensitivityEstimator)
register_sensitivity_estimator('ensemble', CompositeSensitivityEstimator)
