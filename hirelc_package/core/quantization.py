...
class AdvancedQuantizer:
    ...
    @staticmethod
    def quantize_log(tensor: torch.Tensor, bits: int, base: float = 2.0) -> torch.Tensor:
        """Alias for logarithmic quantization (paper compatibility)."""
        return AdvancedQuantizer.quantize_logarithmic(tensor, bits, base=base)
    ...
    def quantize(tensor: torch.Tensor,
                 ...
                 **kwargs) -> torch.Tensor:
        """
        Unified quantization dispatcher.

        Args:
            tensor: Input tensor
            bits: Bitwidth
            mode: 'uniform', 'log', 'per-channel', 'learned'
            quant_type: 'INT' or 'FLOAT' (controls symmetric vs asymmetric)
        """
        if bits == 32:
            return tensor

        mode = (mode or 'uniform').lower()
        symmetric = (str(quant_type).upper() == 'INT')

        if mode in ('uniform', 'uni'):
            return AdvancedQuantizer.quantize_uniform(tensor, bits, symmetric=symmetric)
        if mode in ('log', 'logarithmic'):
            return AdvancedQuantizer.quantize_logarithmic(tensor, bits)
        if mode in ('per-channel', 'per_channel', 'channel'):
            axis = kwargs.get('axis', 0)
            return AdvancedQuantizer.quantize_per_channel(tensor, bits, axis=axis)
        if mode in ('learned', 'trainable'):
            q_tensor, _, _ = AdvancedQuantizer.quantize_learned(tensor, bits)
            return q_tensor
